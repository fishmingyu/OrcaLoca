...
def gather_search_result():
    ...
    for i, inst in enumerate(ds):
        ...
        bug_locations_raw: list = search_output["bug_locations"]
        logger.info(f"Search bug_locations: {bug_locations_raw}")
        bug_locations = [BugLocations.model_validate(x) for x in bug_locations_raw]
        ret = {}
        ...
        for bug in bug_locations:
            if bug.method_name == "":
                loc_info = search_manager._get_exact_loc(
                    f"{bug.file_name}::{bug.class_name}"
                )
            elif bug.class_name == "":
                loc_info = search_manager._get_exact_loc(
                    f"{bug.file_name}::{bug.method_name}"
                )
            else:
                loc_info = search_manager._get_exact_loc(
                    f"{bug.file_name}::{bug.class_name}::{bug.method_name}"
                )
            if loc_info is None:
